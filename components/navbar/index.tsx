    ...
    <>
      ...
      >
        <div className="flex pr-10 items-center justify-center gap-5">
          <Link href={ROUTES.HOME} className=" transform duration-300 ">
            <Image src={logo} alt="Namat Logo" width={50} height={50} />
          </Link>
        </div>
        <nav className=" px-2 lg:px-4 flex justify-between bg-secondary py-2 gap-10 lg:gap-25 items-center text shadow-sm mt-5  rounded-full border shadow-lg ">
          <NavList />
        </nav>
        <Link
          href={ROUTES.HOME}
